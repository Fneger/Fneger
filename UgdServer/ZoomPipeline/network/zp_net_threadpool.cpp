...
namespace ZPNetwork{
        ...
        QObject(parent)
	{
        ...
        qRegisterMetaType<session_ptr>("session_ptr");
        qRegisterMetaType<session_ptr>("session_ptr&");
    ...
    int zp_net_Engine::RabishCanSockets(int idxThread)
    {
        int nsz = m_vec_NetTransThreads.size();
        if (idxThread >=0 && idxThread<nsz)
            return m_vec_NetTransThreads[idxThread]->RabishCanSockets();
        else
            return 0;
    }

    int zp_net_Engine::NewClientSockets(int idxThread)
    {
        int nsz = m_vec_NetTransThreads.size();
        if (idxThread >=0 && idxThread<nsz)
            return m_vec_NetTransThreads[idxThread]->NewClientSockets();
        else
            return 0;
    }
	...
	void zp_net_Engine::AddListeningAddress(QString  id,const QHostAddress & address , quint16 nPort,bool bSSLConn /*= true*/)
    {
		...
		{
			//Start Thread
            QThread * pThread = new QThread(nullptr);
            ...
            Q_EMIT startListen(id);
		}
		else
            qWarning() << "This ID has been used.";
	}
	 ...
	 */
    void zp_net_Engine::on_New_Arrived_Client(session_ptr session)
    {
		zp_netListenThread * pSource = qobject_cast<zp_netListenThread *>(sender());
		if (!pSource)
		{
            qWarning() << "Non-zp_netListenThread type detected.";
			return;
		}

        //Q_EMIT evt_Message(this,"Info>" +  QString(tr("Incomming client arriverd.")));
		...
		if (nMinIdx>=0 && nMinIdx<nsz)
            Q_EMIT evt_EstablishConnection(m_vec_NetTransThreads[nMinIdx],session);
		else
        {
	...
	void zp_net_Engine::AddClientTransThreads(int nThreads,bool bSSL)
    {
		if (nThreads>0 && nThreads<256)
		{
			for (int i=0;i<nThreads;i++)
			{
                ...
                connect (this,&zp_net_Engine::evt_KickRabishCanClient,clientTH,&zp_netTransThread::KickRabishCanTimeOutObj,Qt::QueuedConnection);
                connect (this,&zp_net_Engine::evt_AddNewClientOk,clientTH,&zp_netTransThread::AddNewClientOk,Qt::QueuedConnection);
                connect (this,&zp_net_Engine::evt_EmptyRabishCan,clientTH,&zp_netTransThread::EmptyRabishCan,Qt::QueuedConnection);
	...
	bool zp_net_Engine::TransThreadDel(zp_netTransThread * pThreadObj)
	{
		...
		if (idx>=0 && idx <nsz)
		{
            ...
            Q_EMIT evt_EmptyRabishCan(clientTH);
            qApp->processEvents();
            ...
            disconnect (this,&zp_net_Engine::evt_KickRabishCanClient,clientTH,&zp_netTransThread::KickRabishCanTimeOutObj);
            disconnect (this,&zp_net_Engine::evt_AddNewClientOk,clientTH,&zp_netTransThread::AddNewClientOk);
            disconnect (this,&zp_net_Engine::evt_EmptyRabishCan,clientTH,&zp_netTransThread::EmptyRabishCan);
	...
	void zp_net_Engine::KickAllClients()
	{
		...
		int nsz = m_vec_NetTransThreads.size();
		for (int i=0;i<nsz;i++)
            Q_EMIT evt_KickAll(m_vec_NetTransThreads[i]);
		//m_mutex_trans.unlock();
	}
	/**
	...
	void zp_net_Engine::DeactiveImmediately()
	{
		...
		int nsz = m_vec_NetTransThreads.size();
		for (int i=0;i<nsz;i++)
            Q_EMIT evt_DeactivteImmediately(m_vec_NetTransThreads[i]);
		//m_mutex_trans.unlock();
	}

    void zp_net_Engine::KickRabishCanClient()
    {
        Q_EMIT evt_KickRabishCanClient();
    }
	...
	{
        Q_EMIT evt_SendDataToClient(objClient,dtarray);
	}
	...
	bool zp_net_Engine::connectTo (const QHostAddress & address , quint16 nPort,bool bSSLConn, quint64 extraData)
	{
		...
		{
			res = true;
            Q_EMIT evt_FireConnection(m_vec_NetTransThreads[nMinIdx],address,nPort,extraData);
		}
		else
		{
            qCritical() << "Need Trans Thread Object for clients.";
		}
		//m_mutex_trans.unlock();
		return res;
	}
