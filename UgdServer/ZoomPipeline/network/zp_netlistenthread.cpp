#include "zp_netlistenthread.h"
#include "csocketsession.h"
#include "zp_tcpserver.h"

namespace ZPNetwork{
	zp_netListenThread::zp_netListenThread(QString  id, QHostAddress address ,quint16 port,bool bSSL,QObject *parent)
        :QObject(parent)
		,m_id(id)
		,m_address(address)
		,m_port(port)
		,m_bSSLConn (bSSL)
	{

	}

	bool zp_netListenThread::bSSLConn()
	{
		return m_bSSLConn;
	}

	/**
	 * @brief The slot recieves startListen command from its parent
	 * zp_net_Engine object. the Qt's signal-and-slots system
	 * likes a broadcast system, so, a string id should be used to
	 * tell each listening zp_netListenThread, only this id should be
	 * notified.
	 *
	 * @param id the Listening ID that zp_net_Engine really
	 * want to operate.
	 */
	void zp_netListenThread::startListen(QString  id)
	{
		if (id==m_id)
		{
			if (!m_tcpServer)
			{
                m_tcpServer = boost::make_shared<ZP_TcpServer>();
                m_tcpServer->init();
                //m_tcpServer->setThreadSize(8);

                connect (m_tcpServer.get(),&ZP_TcpServer::evt_NewClientArrived,this,&zp_netListenThread::evt_NewClientArrived,Qt::QueuedConnection);
                if (false==m_tcpServer->listen(m_address.toString(),m_port))
				{
                    disconnect (m_tcpServer.get(),&ZP_TcpServer::evt_NewClientArrived,this,&zp_netListenThread::evt_NewClientArrived);
                    Q_EMIT evt_Message(this,"Error>"+QString(tr("Can not start listen!")));
					//Close this thread.
                    Q_EMIT evt_ListenClosed(m_id);
                }
			}
		}
	}

	/**
	 * @brief this method is very similar to zp_netListenThread::startListen(QString  id)
	 *
	 * @param id the Listening ID that zp_net_Engine really want to operate.
	 */
	void zp_netListenThread::stopListen(QString  id)
	{
		if (id==m_id)
		{
			if (m_tcpServer)
			{
                disconnect (m_tcpServer.get(),&ZP_TcpServer::evt_NewClientArrived,this,&zp_netListenThread::evt_NewClientArrived);
                m_tcpServer->close();
				//Close this thread.
                Q_EMIT evt_Message(this,"Info>"+QString(tr("Listen Closed!")));
                Q_EMIT evt_ListenClosed(m_id);
			}
		}
	}
}
