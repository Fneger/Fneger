...
namespace ZPNetwork{
    ...
    public:
        ...
        void KickRabishCanClient();
        int RabishCanSockets(int idxThread);
        int NewClientSockets(int idxThread);
    ...
    Q_SIGNALS:
        ...
        void evt_NewClientConnected(QObject * /*clientHandle*/, QByteArray, quint64 extraData);
		//SSL Connections OK
        ...
        void evt_KickRabishCanClient();
        void evt_AddNewClientOk(QObject *pSock);
        void evt_EmptyRabishCan(QObject *pTransObj);

		//Internal Message for ctrl.------------------------------------
		//Listen Control
		void startListen(QString  id);
		void stopListen(QString  id);
        void evt_EstablishConnection(QObject * threadid,session_ptr session);
	...
	protected slots:
        void on_New_Arrived_Client(session_ptr session);
