	...
	int zp_ClusterNode::filter_message(QByteArray  block, int offset)
	{
		...
		while (blocklen>offset)
		{
			...
			else
			{
                ...
                Q_EMIT evt_Message(this,tr("Client Send a unknown start Header %1 %2. Close client immediately.")
...
//				Q_EMIT evt_Message(this,"Debug:" + m_currentBlock.toHex().left(64) + "..." + m_currentBlock.toHex().right(64));
//			else
//				Q_EMIT evt_Message(this,"Debug:" + m_currentBlock.toHex());
			...
			if (bytesLeft==0)
			{
				...
				if (strName != m_pTerm->name())
				{
					...
					if (false==m_pTerm->regisitNewServer(this))
					{
                        ...
                        Q_EMIT evt_Message(this,tr("Info: New Svr already regisited. Ignored.")+strName);
                        Q_EMIT evt_close_client(this->sock());
					}
					else
					{
                        Q_EMIT evt_NewSvrConnected(this->termName());
						m_pTerm->BroadcastServers();
					}
				}
				else
				{
                    Q_EMIT evt_Message(this,tr("Can not connect to it-self, Loopback connections is forbidden."));
                    Q_EMIT evt_close_client(this->sock());
				}
			}
			break;
			...
			if (bytesLeft==0)
			{
				...
				for (int i=0;i<nSvrs;i++)
				{
					...
					if (strName != m_pTerm->name() && m_pTerm->SvrNodeFromName(strName)==NULL)
					{
						...
						if (strName > m_pTerm->name())
                            Q_EMIT evt_connect_to(addrToConnectTo,PortToConnectTo,false,0);
						else
                            Q_EMIT evt_Message(this,tr("Name %1 <= %2, omitted.").arg(strName).arg(m_pTerm->name()));
					}
				}
			}
			break;
...
//				Q_EMIT evt_Message(this,"Debug:" + m_currentBlock.toHex());
			...
			break;
		default:
            Q_EMIT evt_Message(this,tr("Info:Unknown Msg Type got."));
            Q_EMIT evt_close_client(this->sock());
			break;
		};
	...
	{
        Q_EMIT evt_RemoteData_recieved(this->termName(),data);
		return true;
	}
	...
	void zp_ClusterNode::CheckHeartBeating()
	{
		...
		if (usc >= nThredHold)
		{
            Q_EMIT evt_Message(this,tr("Client ") + QString("%1").arg((unsigned int)((quint64)this)) + tr(" is dead, kick out."));
            Q_EMIT evt_close_client(this->sock());
		}
	}
	void zp_ClusterNode::SendHelloPackage()
	{
        ...
        Q_EMIT evt_SendDataToClient(sock(),array);
	}
}
